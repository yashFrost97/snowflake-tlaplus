...
--algorithm atomic{
    ...
    LOOOP: while(decision # TRUE){
            if(colors[self] # 2){
                ...
                Respond(self, colors[self], rounds);
    WAIT:   await(Cardinality(RoundMsg(rounds)) > K);
...
WAIT(self) == /\ pc[self] = "WAIT"
              /\ (Cardinality(RoundMsg(rounds[self])) > K)
...
\* Last modified Thu May 21 21:48:12 EDT 2020 by yashf
