...
--algorithm atomic{
    ...
    LOOOP: while(decision # TRUE){
     ...
     WAIT:  if(self = 1){
                await(Cardinality(RoundMsg(rounds)) > K);
                
    CONVI:      with(c \in {0}){
                    if(Cardinality(ColorCounter(c, rounds)) \geq Alpha){
                        majority := TRUE;
                        if(c # colors[self]){
                            colors[self] := c;
                            conviction := 1;
...
LOOOP(self) == /\ pc[self] = "LOOOP"
               ...
               /\ UNCHANGED << query, rounds, count, decided, decision, 
                               majority, conviction >>

WAIT(self) == /\ pc[self] = "WAIT"
              /\ IF self = 1
                    THEN /\ (Cardinality(RoundMsg(rounds[self])) > K)
                         /\ pc' = [pc EXCEPT ![self] = "CONVI"]
                    ELSE /\ pc' = [pc EXCEPT ![self] = "NEXTR"]
              /\ UNCHANGED << colors, counter, query, rounds, count, decided, 
                              decision, majority, conviction >>

CONVI(self) == /\ pc[self] = "CONVI"
               /\ \E c \in {0}:
                    IF Cardinality(ColorCounter(c, rounds[self])) \geq Alpha
...
\* Last modified Fri May 22 00:53:53 EDT 2020 by yashf
